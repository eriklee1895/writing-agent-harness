...
openspeech 端点（`openspeech.bytedance.com/api/v3/tts/create`）只接受以下命名，wiki ONE PAGE 写的方舟命名在此端点会报 `unmapped key audio_setting` 错误：

| 参数 | openspeech 端点（实测可用） | 飞书 wiki 写的（此端点不可用） |
|---|---|---|
| model | `seed-audio-1.0` | `doubao-seed-audio-1-0` |
| 文本 | `text_prompt` | `text` |
| 配置 | `audio_config` | `audio_setting`/`voice_setting` |

**以 openspeech 端点实测版本为准**。wiki 的方舟命名可能对应 `ark.cn-beijing` 端点，未测，后续若方舟端点开放再适配。
