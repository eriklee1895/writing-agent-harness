...
- API 端点固定 `https://openspeech.bytedance.com/api/v3/tts/create`，body 用 `{model, text_prompt, references, audio_config, watermark}`（官方文档方舟命名 `doubao-seed-audio-1-0`/`text`/`audio_setting` 在此端点不可用，实测确认）
...
- 完整 prompt 示例（从官方公开文档 搬护肤广告 / 雨夜告别示例）
