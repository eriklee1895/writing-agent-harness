...
```rust
// codex-rs/app-server/src/request_processors/turn_processor.rs:782-889（节选）
async fn turn_steer_inner(&self, request_id, params: TurnSteerParams)
    -> Result<TurnSteerResponse, JSONRPCErrorError> {
    // 1. 加载并验证 thread
    let (_, thread) = self.load_thread(&params.thread_id).await?;
    self.ensure_direct_input_allowed(request_id, thread.as_ref()).await?;

    // 2. 校验 turn_id 非空、输入大小合法
    if params.expected_turn_id.is_empty() { return Err(invalid_request("...")); }
    Self::validate_v2_input_limit(&params.input)?;

    // 3. 映射输入并转发到核心 thread（错误细分为 4 类，见 §2.5）
    let mapped_items = params.input.into_iter().map(V2UserInput::into_core).collect();
    let turn_id = thread.steer_input(
        mapped_items, map_additional_context(params.additional_context),
        Some(&params.expected_turn_id), ...).await?;
...
```rust
// codex-rs/core/src/session/mod.rs:3240-3313（节选：3 道守卫 + 队列注入）
pub async fn steer_input(&self, input: Vec<UserInput>, additional_context, expected_turn_id, ...)
    -> Result<String, SteerInputError> {
    let mut active = self.active_turn.lock().await;
    let Some(active_turn) = active.as_mut() else { return Err(NoActiveTurn(input)); };
    let active_turn_id = &active_turn.task.as_ref()?.turn_context.sub_id;

    // 守卫 1 · 乐观并发：客户端声明的 turn 必须 == 当前活跃 turn
    if let Some(exp) = expected_turn_id && exp != active_turn_id {
        return Err(ExpectedTurnMismatch { expected: exp.into(), actual: active_turn_id.clone() });
    }
    // 守卫 2 · Turn 类型：Review / Compact 不可 steer
    match active_task.kind {
        TaskKind::Regular => {}
        TaskKind::Review  => return Err(ActiveTurnNotSteerable { turn_kind: Review }),
        TaskKind::Compact => return Err(ActiveTurnNotSteerable { turn_kind: Compact }),
    }
    // 守卫 3 · 空输入
    if input.is_empty() { return Err(EmptyInput); }

    // 合并 additional_context → 构建 pending 队列 → 推入活跃 turn 的 mailbox
    let mut pending_input = /* additional_context 转 ResponseItem */ ...;
    pending_input.push(TurnInput::UserInput { content: input, client_id: client_user_message_id });
    self.input_queue
        .extend_pending_input_and_accept_mailbox_delivery_for_turn_state(
            active_turn.turn_state.as_ref(), pending_input).await;
...
class AIAgent:
    ...
    def steer(self, text: str) -> bool:
        """暂存用户消息，等当前 tool batch 完成后追加到最后一个 tool result（不中断执行）。"""
        if not text or not text.strip():
            return False
        with self._pending_steer_lock:
            self._pending_steer = (self._pending_steer + "\n" + text.strip()
                                   if self._pending_steer else text.strip())
        return True
...
```python
# agent/agent_runtime_helpers.py:2371-2432（节选）
def apply_pending_steer_to_tool_results(agent, messages, num_tool_msgs) -> None:
    """tool batch 结束、下次 API 调用之前，把 pending steer 追加到最后一个 tool result。"""
    ...
    # 在尾部 num_tool_msgs 条里找最后一个 role:"tool" 消息
    target_idx = next(
        (j for j in range(len(messages) - 1, max(len(messages) - num_tool_msgs - 1, -1), -1)
         if messages[j].get("role") == "tool"), None)
    if target_idx is None:        # 本 batch 无 tool result → 放回缓冲区，走 fallback
        return

    # 追加带标记的 steer，让模型知道这来自用户而非 tool
...
![Codex 协议级 Steering 时序图](assets/codex-steer-sequence.png)

*图 7：Codex 协议级 Steering 时序 —— `turn/steer` 请求经 App Server 校验后进入 Session 状态机，依次完成并发守卫、Turn 类型校验、上下文合并与队列注入，Agent loop 在下一次迭代消费排队输入。*

#### Hermes：应用级 Steering

![Hermes 应用级 Steering 时序图](assets/hermes-steer-sequence.png)

*图 8：Hermes 应用级 Steering 时序 —— `steer()` 仅把文本加锁写入缓冲区即返回；Agent Loop 在 tool 执行后 drain 缓冲区，将 steer 追加到最后一条 tool 消息，模型在下一次 API 调用时把它当作 tool 输出读取。*
