...
cover: "../../origin/2026-08-20-agent-evidence/assets/00-cover-agent-evidence.png"
---
...
> **一句话总结：** Agent 的能力决定它能走多远，证据决定我们敢让它走多远。
...
**目标与授权 → 计划与决策 → 工具调用 → 环境返回 → 产物 → 验证 → 回执、暂停或恢复。**
...
一个能落地的最小合同，至少要留下七类信息：

1. **目标**：Agent 被要求完成什么；
2. **授权**：谁批准了这次运行，范围包括哪些 workspace、工具和网络；
3. **轨迹**：调用过哪些工具，返回了什么，失败和重试发生在哪里；
4. **数据处理**：读过哪些敏感数据，保留多久，能否进入训练或评测；
5. **产物**：diff、文件、报告和外部系统回执；
6. **验证**：测试、断言、人工检查和已知缺口；
7. **恢复**：最后一个安全 checkpoint、继续方式和可撤销路径。
