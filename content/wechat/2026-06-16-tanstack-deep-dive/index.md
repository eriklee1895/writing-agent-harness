...
description: "一个为 developer 和 agent 同时工作的 TypeScript 生态——它跟 Next.js 是什么关系、为什么火、怎么用。"
author: Erik
date: 2026-06-16
tags: [TanStack, TypeScript, React, AI Agent, Web Framework]
channel: wechat
style: warm-editorial
cover: ../../origin/2026-06-16-tanstack-deep-dive/assets/20260616-095914-tanstack-cover-cropped.png
---

![TanStack 头图：精密工具 + 薄荷绿点缀 + 背景编辑器](../../origin/2026-06-16-tanstack-deep-dive/assets/20260616-095914-tanstack-cover.png)
...
> 本文是一篇技术学习笔记 + 工程分析。我会先回答"TanStack 到底是什么、为什么火、跟 Next.js 是什么关系"，然后带你跑一段真实可执行的 demo。文章里所有数字、库名、状态都核对自 tanstack.com（截至 2026-06-16）。
...
几个量级数字（截至 2026-06-16，来自 tanstack.com 主页）：
...
下图是 TanStack 当前的全部库、所在层、以及**它们之间的依赖关系**（实线 = "built on / extends"，虚线 = "integrates with / optional"）。来自 tanstack.com 主页抓取，结合官方文档梳理。

![TanStack 生态依赖图：5 个 swimlane + 13+ 库 + 依赖箭头 + 11.3B 下载量统计](../../origin/2026-06-16-tanstack-deep-dive/assets/20260616-184442-tanstack-ecosystem-dependencies.png)
...
tanstack.com 主页开头那句 "work naturally for developers and reliably for agents" 不是 brand 装饰。

### 显式证据 #2：TanStack Builder（Alpha）

访问 tanstack.com/builder，你可以用自然语言描述你要做的 app，Builder 会输出：
...
![TanStack Builder 实际 UI：左边 prompt 输入 + 库/集成选择，右边自动生成 agent prompt](../../origin/2026-06-16-tanstack-deep-dive/assets/tanstack-builder-screenshot.png)
...
### 显式证据 #3：TanStack AI（Beta）

2026-06-09 发布的官方博客把 TanStack AI 的定位讲得很清楚：
...
![TanStack Agent Stack 架构图：LLM → TanStack AI → Server Logic → Data Layer → Router → UI Runtime](../../origin/2026-06-16-tanstack-deep-dive/assets/20260616-100019-tanstack-agent-stack.png)
...
### TanStack 在 Agent 栈里到底是哪一层

为了避免和 LangChain / Hermes Agent 这一层混淆，列一个分层：

- **模型 / 推理**：OpenAI、Anthropic、Ollama——TanStack 不在
- **Agent runtime**：LangGraph、Claude Agent SDK、Hermes Agent——TanStack 不在
...
![TanStack Query 4 步生命周期：Fetch / Share / Revalidate / Collect](../../origin/2026-06-16-tanstack-deep-dive/assets/20260616-100020-tanstack-query-lifecycle.png)
...
官方 comparison 表把三家的 capability 列得很细。摘一些对工程决策有意义的：

![Router vs Next.js vs React Router 对比矩阵](../../origin/2026-06-16-tanstack-deep-dive/assets/20260616-100022-router-vs-next-js.png)
...
- **路由底座**：TanStack Start 用 TanStack Router；Next.js 用自有 App Router；Remix/React Router 7 用 React Router 7 framework mode
- **Server runtime**：Start 用 Vite + Nitro（多 runtime）；Next.js 自有 runtime；Remix 自有 runtime
- **数据加载**：Start 是 Router loader + Query；Next.js 是 RSC + fetch；Remix 是 Loader + Action
- **部署**：Start 任意 Node/edge（Cloudflare、Vercel…）；Next.js 是 Vercel 一等公民；Remix 任意 Node
- **生态成熟度**：Start 是 RC；Next.js 和 Remix 都是 Production-ready
- **卖点**：Start 类型/灵活性/框架无关；Next.js RSC/平台集成/SEO；Remix 渐进增强/Web 标准

LogRocket 的 TanStack Start vs Next.js 文章给了一个相对中立的选型框架：
...
![TanStack 与 Next.js 的分层关系：应用层 → 框架层 → 基础能力层，TanStack 库在底层被任何框架按需选用](../../origin/2026-06-16-tanstack-deep-dive/assets/20260616-184113-tanstack-layered-relationship.png)
...
Reddit r/reactjs 上一位高赞回答基本也是这个分法。

---

## 七、为什么火：4 个核心原则

下图是 TanStack 持续吸引人的 4 个产品原则（tanstack.com/tenets 整理）：

![Why TanStack Wins：4 个核心原则 + 11.3B 下载 / 391M 周下载 / 124K stars 数字](../../origin/2026-06-16-tanstack-deep-dive/assets/20260616-100251-why-tanstack-wins.png)
...
- 官网：tanstack.com
- Query 文档：tanstack.com/query/latest
- Router 文档：tanstack.com/router/latest
- Start 文档：tanstack.com/start/latest
- AI 文档：tanstack.com/ai/latest
- Builder：tanstack.com/builder
- 官方 Router 对比表：tanstack.com/router/latest/docs/comparison
- TanStack AI Beta 博客（2026-06-09）：tanstack.com/blog/tanstack-ai-beta
- TanStack Table v9 TypeScript 性能博客（2026-06-14）：tanstack.com/blog/tanstack-table-v9-typescript-performance
- LogRocket 第三方对比：blog.logrocket.com/tanstack-start-vs-next-js-choosing-the-right-full-stack-react-framework

> 本文是 Erik 的 AI 写作 harness 产出的学习笔记，所有数据均核对自 tanstack.com 截至 2026-06-16。如果你也在评估 TanStack，建议从 Query 或 Router 单点切入，不要试图一次上全套。
