...
tags: ["ai-agent", "agent-runtime", "observability", "evaluation", "security", "harness"]
register: "agent-ai-essay"
summary: "Agent 的能力决定它能走多远，证据决定我们敢让它走多远。"
cover: assets/00-cover-agent-evidence.png
---

# Agent 开始干活之后，最值钱的不是聪明，是证据

![一个 Agent 站在由日志、产物和验证结果组成的证据轨道旁](assets/00-cover-agent-evidence.png)
...
![Agent 展示一份漂亮的结果，但桌面上缺少真正能证明过程的回执和原始材料](assets/01-done-is-not-proof.png)
...
![一个透明的 Agent runtime 走廊，工具调用、上下文注入和子任务调度像连续事件帧一样被保存下来](assets/03-trace-and-replay.png)
...
![一条看似普通的文字流中隐藏着统计信号，水印能够提示来源，却不能替代作者身份和事实核验](assets/04-provenance-watermark.png)
...
![一个本应封闭的 Agent 沙箱，边缘却出现通往网络、凭证和外部系统的意外通道，画面强调防御与审计而非攻击](assets/05-boundary-and-egress.png)
...
![多个专业 Agent 通过开放协议桥交换带有授权、来源和验证结果的工作包](assets/06-agent-handoff.png)
...
![人类通过调节自主性旋钮，决定 Agent 能走多远；每一步都由测试产物、checkpoint 和回执支撑](assets/07-autonomy-evidence.png)
