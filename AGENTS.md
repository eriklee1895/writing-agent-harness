...
- 本地 Markdown 转写飞书云文档使用 user-level skill `markdown-article-to-feishu-doc`（由 erik-agent-skills 维护，不在本 repo）；解析 frontmatter、保留完整 block 结构、本地图片按原始尺寸上传、` ```mermaid ` 自动渲染为画板、`==高亮文本==` 转 callout。火山引擎文档抓取走 user-level `volcengine-doc-fetcher`；联网搜索（中文内容/国内信息优先）使用 project skill `volcengine-web-search`。
