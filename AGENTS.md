...
- Erik Lee 个人博客发布流程使用 project skill：`eriklee-blog-publish-workflow`；负责把 `content/origin/` 正式稿同步到 `/Users/eriklee/code/my_project/eriklee-blog`，检查 assets / taxonomy / build，并按 Cloudflare Pages 自动部署边界处理 git handoff。`git push main` 等价于公开发布，默认需要明确确认。
