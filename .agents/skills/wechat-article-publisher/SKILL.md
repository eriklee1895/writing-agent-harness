...
- **封面图：默认手动**。微信封面是自定义的「拖拽/选图 + 裁剪 + 确认」多步模态（本地上传走自定义拖拽控件 + 异步图库上传），自动化未能稳定完成裁剪/确认，实测停在 `uploaded-unconfirmed`（图传了但封面没落地）。所以**封面默认不自动化**，在草稿箱 final review 时手动设置（你本来就要过一遍）。`--try-cover` 可实验性尝试（点封面区→本地上传→等加载→完成），但**不保证成功**，且只在你显式加该 flag 时才跑（避免每次发布白等 ~25s）；无论成败都不阻塞草稿保存。封面 UI 结构 dump 在 `scripts/.artifacts/cover-ui.html`。提示：弹窗里有「从正文选择」，若封面图同时是正文首图，未来可走这条更稳的路。
