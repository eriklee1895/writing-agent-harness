...
# Multi-character cloning: up to 3 reference audios, bound in flag order
uv run scripts/seed-audio-gen.py '主播甲（饰演者为 <<TGT_SPK1>>）说："我是一号声音。"主播乙（饰演者为 <<TGT_SPK2>>）笑着回应："我是二号声音。"' \
...
| `--ref-audio <path-or-url>` | Reference audio for voice cloning: local path or `http(s)` URL (auto-detected). **Repeat up to 3 times** for multi-character cloning. Each clip ≤30s, ≤10MB. Bind in the prompt with `<<TGT_SPK1>>`..`<<TGT_SPK3>>`, in the same order as the flags |

`--speaker` and `--ref-audio` are mutually exclusive (pick one voice source). Multiple `--ref-audio` flags are allowed (max 3).

| Flag | Description |
|---|---|
| `--ref-image <path>` | Local reference image path (auto base64-encoded, max 10MB) — generates audio matching the picture's atmosphere/character setup. API support verified 2026-08-27 |
| `--ref-image-url <url>` | Remote reference image URL |

Image references **cannot be mixed with audio references** (API error `45001001: image reference cannot be mixed with audio or video references`; the CLI pre-validates this).
...
For the full prompt-writing guide — timestamp syntax, total-duration declaration, scene element structure, `<<TGT_SPKN>>` multi-voice binding, directing vocabulary, voice selection, and complete worked examples — read `references/seedaudio-prompt-guide.md`.
...
| 多角色音色克隆 | 2-3 条参考音频 + `<<TGT_SPKN>>` 绑定 | Multi-Reference section |
...
2. Generate segment 2 with `--ref-audio segment1.mp3` (or the tail few seconds of it) — the model treats the previous output as a voice reference and extends with the same timbre. For multi-character scenes, pass each character's reference audio again in the same order, keeping `<<TGT_SPKN>>` bindings identical.
