...
3. `<cwd>/.baoyu-skills/.env` with `WECHAT_APP_ID=...` (legacy)
4. `$HOME/.baoyu-skills/.env` with `WECHAT_APP_ID=...`
...
Where to save?
A) Project-level: .env (this repo only; recommended for writing-agent-harness)
