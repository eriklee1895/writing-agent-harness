...
3. repo root `.env` with the prefixed key `WECHAT_{ALIAS}_APP_ID`
4. `.baoyu-skills/.env` with the prefixed key `WECHAT_{ALIAS}_APP_ID` (legacy)
5. `~/.baoyu-skills/.env` with the prefixed key
