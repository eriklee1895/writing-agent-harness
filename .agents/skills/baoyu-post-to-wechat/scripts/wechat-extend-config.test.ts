...
async function writeRootEnvFile(root: string, content: string): Promise<void> {
...
test("loadCredentials prefers repo root .env over legacy project .baoyu-skills env", async (t) => {
  const cwdRoot = await makeTempDir("wechat-creds-cwd-");
  const homeRoot = await makeTempDir("wechat-creds-home-");
  ...
  await writeRootEnvFile(cwdRoot, "WECHAT_APP_ID=root-app-id\nWECHAT_APP_SECRET=root-app-secret\n");
  await writeEnvFile(cwdRoot, "WECHAT_APP_ID=legacy-app-id\nWECHAT_APP_SECRET=legacy-app-secret\n");
  await writeEnvFile(homeRoot, "WECHAT_APP_ID=home-app-id\nWECHAT_APP_SECRET=home-app-secret\n");

  const credentials = loadCredentials();

  assert.equal(credentials.appId, "root-app-id");
  assert.equal(credentials.appSecret, "root-app-secret");
