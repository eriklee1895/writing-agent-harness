...
function resolveCredentialSource(
  ...
  throw new Error(
    ...
    "Set via EXTEND.md account config, environment variables, repo root .env, or .baoyu-skills/.env file." +
    partialHint
  );
}

export function loadCredentials(account?: ResolvedAccount): LoadedCredentials {
  const cwdRootEnvPath = path.join(process.cwd(), ".env");
  const cwdEnvPath = path.join(process.cwd(), ".baoyu-skills", ".env");
  const homeEnvPath = path.join(homeDir(), ".baoyu-skills", ".env");
  const cwdRootEnv = loadEnvFile(cwdRootEnvPath);
  ...
  if (prefix) {
    ...
    sources.push(
      ...
      buildCredentialSource(`<cwd>/.env (${prefixedKeyLabel})`, cwdRootEnv, `${prefix}APP_ID`, `${prefix}APP_SECRET`),
  ...
  sources.push(
    ...
    buildCredentialSource("<cwd>/.env", cwdRootEnv, "WECHAT_APP_ID", "WECHAT_APP_SECRET"),
