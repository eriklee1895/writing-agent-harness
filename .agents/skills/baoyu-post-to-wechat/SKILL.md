...
|--------|-------|----------|
| `browser` (Recommended for this repo) | Slow | Chrome + logged-in session |
| `api` | Fast | API credentials + local IP allowlisted |
| `remote-api` | Fast | API credentials + an SSH-reachable server whose IP is on the WeChat allowlist |

**API selected + missing credentials** → run guided setup per `references/api-setup.md` (writes to repo root `.env` by default).
