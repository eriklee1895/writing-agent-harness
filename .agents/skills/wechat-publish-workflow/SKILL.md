...
5. **Frontmatter pre-flight（调用 publisher 前必须做）**：读取 `index.md` frontmatter，确认：
   - `title`、`date` 存在；
   - `summary` 存在且 ≤120 字（公众号摘要长度上限）。如果缺失或过长，回到 `polish-article` 流程补/改，**不要**让 publisher 自动截取首句——那会丢失作者判断；
