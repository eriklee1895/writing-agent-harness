...
> **Lite 的 method-2 映射与 Pro 不同**（例如 Lite 2K 16:9 = 2848×1600，Pro 2K 16:9 = 2816×1584），且多 3K/4K 两档。Lite 精确值以[官方文档](https://www.volcengine.com/docs/82379/1541523?lang=zh)为准。
>
> **方式 1 vs 方式 2 何时用哪个**：需要确定像素（平台上传/印刷/匹配既有版式）→ 方式 1（`--size WxH` 或 shortcut）；只描述"感觉"、比例对即可、接受模型判断具体像素 → 方式 2（`--size 2K` + prompt 写"16:9 横版"，输出为该比例的参考像素）。本 skill 的 `--wide`/`--landscape`/`--square` 等 shortcut 是方式 1 的预设精确像素（见下），与方式 2 档位是两套机制。
